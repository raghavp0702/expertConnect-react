...
setCurrentImage
  ...
  const postPicsRef = ref(storage, `backgroundImages/${file.name}`);
  const uploadTask = uploadBytesResumable(postPicsRef, file);
  ...
  uploadTask.on(
    ...
    () => {
      getDownloadURL(uploadTask.snapshot.ref).then((response) => {
        editProfile(id, { backgroundLink: response });
        setModalOpen(false);
        setCurrentImage({});
        setProgress(0);
      });
...
export const uploadResume =(
  ...
  setCurrentImage
    ...
    const postPicsRef = ref(storage, `resume/${file.name}`);
    const uploadTask = uploadBytesResumable(postPicsRef, file);
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((response) => {
          editProfile(id, { resumeLink: response });
          setModalOpen(false);
          setCurrentImage({});
          setProgress(0);
        });
